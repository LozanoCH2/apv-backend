...
const nuevoPassword = async (req, res) => {
  ...
  try {
    ...
    res.json({ msg: "Password no se pudo modificar" });
    console.log(veterinario);
  }
};
...
const confirmar = async (req, res) => {
  ...
  try {
    ...
    // Enviar la respuesta solo después de que se haya completado la actualización
...
const actualizarPassword = async (req, res) => {
  ...
  //Comprobar que el veterninaro existe
  const veterinario = await Veterinario.findById(id);
  if (!veterinario) {
    const error = new Error("Hubo un error");
    return res.status(400).json({ msg: error.message });
  }
  //Comprobar tu password
  if (await veterinario.comprobarPassword(pwd_actual)) {
    veterinario.password = pwd_nuevo;
    await veterinario.save();
    res.json({ msg: "Password Alamcenado Correctamente" });
  } else {
